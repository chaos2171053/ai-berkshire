...
**核心原则**：Checklist的目标是**排除坏选择**。对于C级公司，"数据不足"不等于"不通过"，也不等于"否决"——应诚实标注为"灰色地带，需补充一手信息"，而不是因为AI无法填满表格就判为否决。
...
**多家公司逐家顺序执行，不并行启动后台agent。**

| 方式 | 额外内存 | 说明 |
|:----|:--------|:-----|
| ✅ **顺序执行（默认）** | **~0MB** | 每家公司分析完后释放中间数据 |
| ❌ N个并行delegate_task | +N×500MB~1GB | 多公司时Pi不可承受 |

规则：
- 所有工具调用在主会话前台同步执行
- 每家公司独立成章，包含六关评分表+核心数据+镜子测试+明确结论
- 多公司时在末尾统一输出总览对比表
- 不使用后台 agent / 不并行启动子任务

---

### 第二步：逐公司执行六关 Checklist（顺序循环）
