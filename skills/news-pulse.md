...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 /home/chaos/projects/chaos/hermes-agent/packages/ai-berkshire/tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     - 段永平 user_id: `1247347556`
     - 命令示例：`python3 /home/chaos/projects/chaos/hermes-agent/packages/ai-berkshire/tools/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/ai-berkshire/dyp-{公司名}.md`；综合归因后删除该辅助文件
...
不要启动后台 Agent。按 company-event-scout → regulatory-watcher → industry-peer-analyst → sentiment-tracker 的顺序在主会话中执行。每轮使用以下执行模板：
...
不要写入仓库内长期本地文件。完整报告正文先保存到 `/tmp/ai-berkshire/reports/{公司名}/{公司名}-news-{YYYYMMDD}.md`；先执行数据抽检，准出后交给 preview skill 处理。

### 第九步：数据抽检（准出流程）

抽检前将完整报告正文保存到 `/tmp/ai-berkshire/reports/{公司名}/{公司名}-news-{YYYYMMDD}.md`；`report_audit.py --report` 只接受该本地临时路径，不使用 preview/gist 导出路径。准出后将报告正文交给 preview skill；preview 完成后删除该文件。

```bash
# Step 1 — 提取抽检清单（15%随机抽样）
python3 /home/chaos/projects/chaos/hermes-agent/packages/ai-berkshire/tools/report_audit.py extract \
  --report <上一步保存的 /tmp/ai-berkshire/reports/{公司名}/{公司名}-news-{YYYYMMDD}.md 路径>
