...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     ...
     - 命令示例：`python3 tools/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/ai-berkshire/dyp-{公司名}.md`；综合归因后删除该辅助文件
...
不要写入仓库内长期本地文件。完整报告正文先保存到 `/tmp/ai-berkshire/reports/{公司名}/{公司名}-news-{YYYYMMDD}.md`，目录和文件名镜像 `reports/{公司名}/{公司名}-news-{YYYYMMDD}.md`；先执行数据抽检，准出后交给 preview skill 处理。
