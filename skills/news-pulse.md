...
保留以下 4 个任务定义，逐个执行；每轮保留完整时间线和3-5条核心发现，再进入下一轮：
...
### 第五步：顺序执行 4 个 Agent 侦察角色

不要启动后台 Agent。按 company-event-scout → regulatory-watcher → industry-peer-analyst → sentiment-tracker 的顺序在主会话中执行。
