...
- 每篇写完先将文章正文存到临时系列目录：`/tmp/ai-berkshire/reports/{公司名}/《看懂{公司名}》/0X-XX.md`
...
确认无明显问题后，将正文交给 preview skill；如用户后续提出修改意见，再读取对应文章正文继续修订。需要继续修订时保留 `/tmp/ai-berkshire/reports/{公司名}/《看懂{公司名}》/`。
