...
- 每篇写完先将文章正文存到临时系列目录：`/tmp/ai-berkshire/{公司名}/《看懂{公司名}》/0X-XX.md`，目录结构镜像 `reports/{公司名}/《看懂{公司名}》/`，避免引入时间戳/slug/series 层导致后续同步上游时产生额外冲突
...
grep -r "linxuan\|/Users/\|<用户公司花名>" /tmp/ai-berkshire/{公司名}/《看懂{公司名}》/ | head
```

确认无明显问题后，将正文交给 preview skill；如用户后续提出修改意见，再读取对应文章正文继续修订。需要继续修订时保留 `/tmp/ai-berkshire/{公司名}/《看懂{公司名}》/`；系列发布流程结束后删除该临时目录。
