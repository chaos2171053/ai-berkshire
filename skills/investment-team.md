...
对 $ARGUMENTS 进行团队化投资研究分析。保留四个 Agent 角色的独立视角，但在主会话中顺序执行，不创建后台 Agent。
...
**完成后**：
1. 保留完整分析报告作为本角色报告
2. 提取3-5条核心要点，更新进度后进入下一个 Agent 角色
```

### 第五步：接收报告并跟踪进度

- 向用户实时展示进度表（哪些 Agent 角色已完成、哪些仍在研究中）
- 每完成一段角色分析，更新进度并展示该报告的核心要点（3-5条）
- 四段都完成后，再进入汇总，不提前下结论

### 第六步：阶段收束

检查四份报告是否都包含明确结论、关键数据来源和反面证据。缺项先补齐，再汇总。
...
1. **4个 Agent 角色必须顺序执行**——不启动后台 Agent，不并行拆分 LLM 会话
2. **Agent 角色边界要清楚**——每段分析都保留对应大师视角，不要提前合并
3. **数据准确性**——要求使用WebSearch搜索最新数据，关键数据交叉验证
4. **结论要明确**——不回避给出买入/观望/回避建议和具体价格区间
5. **所有分析必须有数据支撑**——附数据来源
6. **进度要透明**——每完成一段 Agent 角色分析，向用户展示核心要点
7. **反偏见意识**——team-lead在汇总时必须评估：各 Agent 角色的分析是否受限于资料充裕度？是否与市场共识过度趋同？最终报告需包含"信息丰富度评级"和"AI研究局限性声明"
