...
**所有角色分析在同一会话中顺序执行。** 每个角色完成后，保留完整分析块和3-5条核心要点，再进入下一个角色；最终由主会话综合判断。

| 方式 | 额外内存 | 说明 |
|:----|:--------|:-----|
| ✅ **顺序执行（默认）** | **~0MB** | 无后台LLM会话残留 |
| ❌ 后台多角色并行 | +500MB~1GB+/会话 | Hermes/Pi 内存不可承受 |

规则：
- 不使用 TeamCreate/TaskCreate/后台 Agent/`run_in_background`
- 不把任务拆给 delegate_task 或其他常驻子进程
- 角色视角和输出要求保持独立，综合时找交叉和矛盾，不把四段分析混成流水账
...
不创建后台团队。你在主会话中担任 team-lead，按下列四个角色顺序执行并记录分析块：

1. business-analyst：商业模式 & 护城河分析（段永平视角）
2. financial-analyst：财务报表 & 估值分析（巴菲特视角）
3. industry-researcher：行业格局 & 竞争态势（芒格视角）
4. risk-assessor：风险评估 & 管理层研判（李录视角）

### 第三步：四个角色任务定义

以下任务逐个执行。每个任务都要有 subject、description，并保留该角色的完整分析输出，供最后汇总使用。
...
不要启动后台 Agent。按 business-analyst → financial-analyst → industry-researcher → risk-assessor 的顺序，在主会话中逐段完成。每个角色使用以下执行模板：
...
将完整最终报告写入 `reports/{公司名}投资研究报告_{日期}.md`（日期格式 YYYYMMDD）。
