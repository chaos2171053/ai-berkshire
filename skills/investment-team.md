...
- 四个角色在主会话中按顺序执行。
- 不使用 TeamCreate、TaskCreate、后台 Agent 或 `run_in_background`。
- 不拆分子任务，不使用 `delegate_task`。
- 每个角色完成后，保留完整分析块和3-5条核心要点，再进入下一个角色。
- 综合时找交叉和矛盾，不把四段分析混成流水账。
...
先将完整最终报告正文保存为本地临时 `.md` 文件（本地临时稿）；`report_audit.py --report` 只接受本地文件路径，不要使用 preview/gist 导出的文件。
