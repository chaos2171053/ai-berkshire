...
**所有步骤在本会话中顺序执行，不拆分后台子进程。** 每个分析段直接输出到最终报告，不保留中间文件。

| 方式 | 额外内存 | 说明 |
|:----|:--------|:-----|
| ✅ **顺序执行（默认）** | **~0MB** | Python工具跑完即释放，无后台残留 |
| ❌ `delegate_task` 子agent | +500MB~1GB+ | 另起LLM会话，完全不推荐 |

规则：
- 所有工具调用在主会话前台同步执行，进程退出即释放内存
- 每层过滤掉的标的必须留下淘汰理由，不能黑箱
...
| 数据 | 来源 |
|:----|:-----|
| 财务数据 | `mx-data` — 东方财富权威数据库（A股） |
| 搜索/新闻 | `mx-search` — 东方财富妙想搜索（A股优先） |
| A股行情 | `ashare_data.py` |
| 估值验算 | `financial_rigor.py` |
| 美股/港股 | mx-search通用搜索 + web_search |
| 年报/季报原文 | `Annualreport_tools` 项目 |
