...
不使用 TeamCreate，不创建后台团队。你在主会话中担任 team-lead，按以下6个角色顺序执行并记录完整研究。

### 第三步：整理6个角色任务

整理以下6个任务（每个都要保留 subject、description、activeForm）：
...
```
你现在以{公司名}未上市公司研究团队中"{角色中文名}"的视角工作。
...
抽检前将完整最终报告正文保存到 `/tmp/ai-berkshire/{公司名}/{公司名}-private-{YYYYMMDD}.md`，目录和文件名镜像 `reports/{公司名}/{公司名}-private-{YYYYMMDD}.md`；`report_audit.py --report` 只接受该本地临时路径，不使用 preview/gist 导出路径。准出后将报告正文交给 preview skill；preview 完成后删除该文件。

```bash
# Step 1 — 提取抽检清单（15%随机抽样）
python3 tools/report_audit.py extract \
  --report <上一步保存的 /tmp/ai-berkshire/{公司名}/{公司名}-private-{YYYYMMDD}.md 路径>
...
1. **6个角色必须顺序执行**——不启动后台 Agent，不并行拆分 LLM 会话
