...
**所有步骤在同一会话中顺序执行，不拆分后台子进程。** 实测内存对比：

| 方式 | 额外内存 | 说明 |
|:----|:--------|:-----|
| ✅ **顺序执行（默认）** | **~0MB** | Python工具跑完即释放，无后台残留 |
| ❌ background子进程 | +30~50MB | Python进程常驻，需手动管理回收 |
| ❌ sub-agent/delegate_task | +500MB~1GB+ | 另起LLM会话，完全不推荐 |

规则：
- 所有Python工具使用前台同步执行，进程退出即释放内存
- 不启动后台进程/不split子任务
- 数据收集步骤的多个独立查询可并行发起，运行完自动回收
...
数据收集完成后，**必须调用 `~/work/hermes-agent/packages/ai-berkshire/tools/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
