...
数据收集完成后，**必须调用 `/home/chaos/projects/chaos/hermes-agent/packages/ai-berkshire/tools/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
...
抽检前将完整报告正文保存到本地临时文件 `<报告文件路径>`；`report_audit.py --report` 只接受该本地临时路径，不使用 preview/gist 导出路径。准出后将报告正文交给 preview skill；preview 完成后删除该文件。
