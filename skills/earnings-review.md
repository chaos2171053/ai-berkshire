...
**所有步骤在本会话中顺序执行，不拆分后台子进程。**

| 方式 | 额外内存 | 说明 |
|:----|:--------|:-----|
| ✅ **顺序执行（默认）** | **~0MB** | 工具调用退出即释放，无后台残留 |
| ❌ `delegate_task` 子agent | +500MB~1GB+ | 完全不推荐 |

规则：
- 所有数据提取通过前台命令执行
- 一手资料优先：财报原文 > 电话会纪要 > 第三方汇总
- 使用 `financial_rigor.py` 对关键数据验算
...
顺序获取以下原始材料（通过 mx-search 搜索链接，或用 Annualreport_tools 下载转换）：

1. **财报原文**：巨潮资讯网（A股）/ SEC EDGAR（美股）/ 港交所披露易（港股）
2. **业绩电话会纪要/录音**：雪球、公司IR页面等
3. **管理层致股东信**（如有年报）：完整阅读
4. **投资者日/分析师日材料**（如近期有）

如果无法获取完整原文，按 `skills/financial-data.md` 规范使用标准数据源拼凑（A股：东方财富+巨潮资讯；美股：macrotrends+stockanalysis；港股：aastocks+macrotrends），但必须标注"非原始财报，来自第三方汇总"，且关键数据两源误差>1%须标记。
...
python3 tools/financial_rigor.py verify-market-cap \
  --price {价格} --shares {股本} --reported {市值} --currency {币种}
...
将报告写入 `reports/{公司名}/{公司名}-earnings-{期间}.md`
